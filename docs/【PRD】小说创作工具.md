...
flowchart TD
    ...
    B -->|否| D[关键词智能识别<br/>查找包含"设定""大纲""概要""内容"的目录]
    D --> E{识别结果}
    E -->|4个关键词全部找到| F[导入成功，加载项目到网站<br/>其他目录自然展示]
    E -->|部分识别或无法识别| G[显示严格格式示例<br/>提示：请手动创建缺失目录后重新导入]
    G --> H[用户查看示例，手动创建目录]
    H --> I[用户重新选择目录导入]
    I --> D
    C --> F
    F --> J[显示项目首页，默认显示小说设定内容]
```

**2. 依赖后置协作流程**
```mermaid
flowchart TD
    A[用户选择任意创作内容] --> B[系统智能识别并检查前置依赖]
    B --> C{缺少必要依赖?}
    C -->|否| D[展示内容，智能路由AI角色]
    C -->|是| E[友好建议：先完成前置依赖内容<br/>（小说设定/对应大纲/文章概要）]
    E --> F{用户选择}
    F -->|先补充依赖| G[跳转到依赖内容创建]
    F -->|继续当前内容| H[允许创建，标记为草稿]
    D --> I[用户与AI角色对话协作]
    G --> I
    H --> I
    I --> J[AI执行修改并保存]
    J --> K[用户可自由切换到任意文章的任意内容类型]
```
...
**双层Tab结构**：
- 顶层Tab：自动创建，支持关闭和拖拽，最多8个
- 内层Tab：智能显示依赖链(设定→对应大纲→概要→当前)，便捷校对
...
**目录结构智能识别**：
- **标准4目录要求**：系统必须识别包含"设定"、"大纲"、"概要"、"内容"关键词的4个核心目录
- **严格示例引导**：向用户展示标准格式 `0-小说设定`、`1-故事大纲`、`2-故事概要`、`3-小说内容`
- **关键词灵活识别**：只要目录名包含对应关键词即可成功识别（如"设定资料"、"故事设定"、"World Setting"等）
...
**左侧交互区**：
- **选项卡切换**：对话(AI角色对话+角色切换按钮) | 项目(4类型导航+文件列表+依赖状态指示)
- **对话界面**：消息滚动区域 + 输入框(角色切换+发送按钮) + 时间戳

**右侧内容区**：
- **顶层Tab**：打开文件自动创建，支持关闭/拖拽，最多8个
- **内层Tab**：智能显示依赖链条便于校对，根据当前文件智能识别并显示前置依赖

**编辑功能**：Markdown富文本编辑 + 实时预览 + 字数统计 + 自动保存状态指示
...
flowchart TD
    ...
    E -->|有内容| G[关键词智能识别<br/>查找"设定""大纲""概要""内容"]
    G --> H{识别完整度}
    H -->|4个关键词全部识别| I[导入成功，加载项目到左侧项目树<br/>其他目录自然展示]
    H -->|部分识别或无法识别| J[显示详细示例对话框<br/>提示：请手动创建缺失目录]
    J --> K[用户查看示例，手动创建目录]
    K --> L[用户重新点击导入]
    L --> C
    F --> I
