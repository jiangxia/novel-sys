...
- **创作流程引导**：小说设定 → 故事大纲 → 故事概要 → 小说内容
- **AI角色引导**：AI角色通过提示词工程识别必需的前置文件，缺少时温和拒绝并建议先创建
- **目录结构映射**：本地文件系统与网页展示的智能映射

**2. AI角色协作模块**
- **4角色系统**：总监（统筹质量）、架构师（世界观构建）、规划师（情节设计）、写手（文本创作）
- **简单角色切换**：用户手动切换角色，或根据创作类型选择对应角色
...
**协作逻辑**：用户选择创作内容 → 用户选择或系统推荐AI角色 → AI角色检查前置文件存在性 → 用户与AI协作优化内容 → 内容编辑器提供实时编辑 → 文件操作模块安全保存到本地
...
**3. 自由探索创作流程**
用户自由选择任意篇章的任意类型内容，选择对应AI角色：小说设定→架构师 | 故事大纲/概要→规划师 | 小说内容→写手 | 跨类型协调→总监，AI角色通过提示词检查前置文件

#### 业务规则集中说明

**AI角色依赖处理**：
- **提示词工程实现**：在AI角色提示词中定义必需的前置文件要求
- **简单拒绝机制**：当前置文件不存在时，AI直接温和拒绝并提示用户先创建
- **角色专业分工**：架构师需要无前置，规划师需要小说设定，写手需要对应概要
- **无复杂状态**：不存在草稿状态，只有存在/不存在两种状态
...
| 角色 | 职责定位 | 触发场景 | 对话特点 |
|------|----------|----------|----------|
| 总监 | 统筹全局、质量把控、跨篇章协调 | 全局建议、篇章衔接、质量评估 | 宏观视角，注重逻辑性和完整性 |
| 架构师 | 世界观构建、设定体系化 | 各篇设定内容主协作、世界观讨论 | 专业深度，体系化和逻辑自洽 |
| 规划师 | 故事结构设计、情节安排 | 各篇故事大纲/概要内容主协作、情节优化 | 结构化思维，逻辑性和可读性 |
| 写手 | 文本创作、语言表达 | 各小说内容创作主协作、文本润色 | 文学性强，注重表达和感染力 |
...
**特殊情况**：前置文件缺失时AI角色直接温和拒绝，超出角色能力时建议切换角色，发现问题时主动提醒
...
1. **提示词检查**：规划师AI角色通过提示词检查小说设定和故事大纲是否存在
...
**用户交互流程**：
1. 用户：指定要创作哪一篇的小说内容
2. 系统：发送请求给AI（包含写手角色提示词 + 对应概要文件内容）
...
1. 用户在左侧项目树点击文件
2. 右侧自动创建新Tab并加载内容
3. 内层Tab根据创作流程自动加载前置文件内容（设定→大纲→概要→当前）
